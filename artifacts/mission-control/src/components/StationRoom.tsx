...
// Bug 1 fix: accepts isAway prop; when true, hides the in-room sprite so it
// doesn't duplicate the CrewVisits overlay sprite.
...
interface StationRoomProps {
  ...
  /** BUG FIX: true when this agent is currently being shown by CrewVisits overlay */
  isAway?: boolean;
}
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTerminalLines(prev => {
        const n = [...prev]; n[Math.floor(Math.random() * n.length)] = `> ${agentId}@station:~$ ${randomCommand()}`; return n;
      });
    }, 2000);
  ...
  return (
    <div className="relative overflow-hidden" data-agent-id={agentId}>
      <div className="relative">
        {useCanvas ? (
          <RoomCanvas room={ROOM_TYPE_MAP[roomId] || "command"} className="w-full h-auto" />
        ) : (
          <img src={bgImage} alt={label} className="w-full h-auto" style={{ imageRendering: "pixelated" }} />
        )}

        <div className="absolute top-1 left-1 font-mono text-[7px] tracking-[0.15em] text-muted-foreground/50 bg-background/40 px-1.5 py-0.5">{label}</div>
        <RoomAmbient roomId={ROOM_TYPE_MAP[roomId] || "command"} accent={accent} intense={true} />
        ...
        {/* Status bubble — always show, even when away */}
        {bubbleText && (
        ...
        {/* BUG FIX: hide sprite when agent is shown by the CrewVisits overlay */}
        ...
        {/* Name / status below sprite — show even when away (agent is out of room) */}

...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// PIXEL ART ENGINE
// Sprites drawn on 40×56 pixel grid, scaled to fill 320×240 frame
// Each "pixel" = PW wide × PH tall → 200×224px centered in 320×240
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const PW = 5, PH = 4;
const GW = 40, GH = 56;
const SPRITE_W = GW * PW; // 200
const SPRITE_H = GH * PH; // 224
const FRAME_W = 320, FRAME_H = 240;
const OX = Math.floor((FRAME_W - SPRITE_W) / 2); // 60
const OY = Math.floor((FRAME_H - SPRITE_H) / 2); // 8

function makeDraw(ctx, fox, foy) {
  const px = (c, r, color, cw = 1, ch = 1) => {
    if (!color || color === ".") return;
    ctx.fillStyle = color;
    ctx.fillRect(fox + OX + c * PW, foy + OY + r * PH, cw * PW, ch * PH);
  };
  const line = (c, r, len, color, vert = false) => {
    for (let i = 0; i < len; i++) vert ? px(c, r + i, color) : px(c + i, r, color);
  };
  const rect = (c, r, w, h, color) => {
    if (!color) return;
    for (let row = 0; row < h; row++) line(c, r + row, w, color);
  };
  const glow = (c, r, w, h, color) => {
    ctx.fillStyle = color;
...
// Dim a hex color for sleep state
function dimHex(col, f) {
...
function drawMonkey(ctx, pose, fox, foy, t) {
  ...
  line(14, bY, 12, c(PM.s2)); // highlight top
  line(15, bY+1, 3, c(PM.s3)); // shine
  // Gold collar trim
  ...
  if (pose === "idle") {
    ...
    rect(10, bY+10, 4, 4, c(PM.fur2)); // hand on console
    ...
    rect(10, bY+12, 5, 4, PM.fur2); // hand on chin
    ...
    line(5, bY+8, 6, PM.s1); rect(4, bY+7, 4, 4, PM.fur2); // pointing
  ...
  if (pose === "working") {
    ...
    px(12, bY-2, c(PM.hm)); // mic active light
  }

  // ── SHARED DISPLAY (collab) ──
  if (pose === "collab") {
  ...
  rect(15, hY+3, 10, 6, c(PM.fur3)); // face lighter
  // Ears
  ...
  px(18, hY+8, c(PM.fur0)); px(19, hY+8, c(PM.fur0)); // nostrils
  if (pose==="collab") {
  ...
  // ── HEADSET ──
  line(14, hY, 12, c(PM.h0)); // band
  rect(12, hY+2, 3, 4, c(PM.h0)); rect(25, hY+2, 3, 4, c(PM.h0)); // earcups
  rect(13, hY+3, 2, 3, c(PM.h1)); rect(26, hY+3, 2, 3, c(PM.h1));
  rect(11, hY+5, 2, 4, c(PM.h0)); // mic arm
  rect(10, hY+8, 3, 2, c(PM.h0)); // mic
  if (!sl) px(10, hY+9, PM.hm); // mic green light
  // Data visor (right eye)
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 🥷 LIFE SUPPORT NINJA DRAWER
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function drawNinja(ctx, pose, fox, foy, t) {
  const { px, line, rect, glow } = makeDraw(ctx, fox, foy);
  const sl = pose === "sleep";
  const f = sl ? 0.45 : 1;
  const c = (col) => sl ? dimHex(col, f) : col;
  ...
  if (pose === "idle") {
    ...
    line(6, bY+8, 8, PN.s2); rect(4, bY+7, 5, 4, PN.s1); // arm+hand pointing
    rect(25, bY+4, 5, 10, PN.s2); rect(26, bY+5, 3, 8, PN.s1); rect(25, bY+13, 4, 4, PN.s1);
  } else { // sleep
    rect(10, bY+4, 5, 8, c(PN.s2)); rect(25, bY+4, 5, 8, c(PN.s2));
  ...
  if (sl) {
    ...
    line(16, hY+2, 4, c(PN.s0)); line(21, hY+2, 4, c(PN.s0)); // eyelid
  }
  // Lower mask
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 🔧 SYSTEMS ENGINEER DRAWER
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function drawEngineer(ctx, pose, fox, foy, t) {
  const { px, line, rect, glow } = makeDraw(ctx, fox, foy);
  const sl = pose === "sleep";
  const f = sl ? 0.45 : 1;
  const c = (col) => sl ? dimHex(col, f) : col;
  ...
  line(14, bY+1, 4, c(PE.cy2)); // shine
  rect(12, bY+3, 2, 4, c(PE.c3)); rect(26, bY+3, 2, 4, c(PE.c3)); // rolled sleeves
  ...
  if (pose === "idle") {
    ...
    rect(5, bY+12, 5, 3, c(PE.rl0)); rect(6, bY+13, 3, 2, c(PE.rl1)); // rag
    ...
    rect(9, bY+12, 5, 4, PE.sk); // hand on chin
    ...
    for (let i=0;i<7;i++) {
      ...
      px(sx, sy, c([PE.sp0,PE.sp1,PE.sp2][i%3]));
    }
    ...
    rect(9, bY+13, 20, 5, c(PE.c2)); // arms on bench
  }
  ...
  px(18, hY+8, c(PE.skD)); px(19, hY+8, c(PE.skD)); // nose
  ...
  line(12, gY, 17, c(PE.og0)); // band
  rect(14, gY, 5, 5, c(PE.og1)); rect(23, gY, 5, 5, c(PE.og1));
  rect(15, gY+1, 3, 3, c(PE.og3)); rect(24, gY+1, 3, 3, c(PE.og3));
  px(25, gY+1, c(PE.og0)); px(26, gY+2, c(PE.og0)); // crack right lens
  line(19, gY+2, 4, c(PE.og0)); // bridge
  if (gogDown && !sl) {
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 📚 STATION ARCHIVIST DRAWER
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function drawArchivist(ctx, pose, fox, foy, t) {
  const { px, line, rect, glow } = makeDraw(ctx, fox, foy);
  const sl = pose === "sleep";
  const f = sl ? 0.42 : 1;
  const c = (col) => sl ? dimHex(col, f) : col;
  ...
  function crystal(cx, cy, col, bright) {
    rect(cx, cy, 2, 2, c(col)); if (!sl) px(cx, cy, bright);
  }
  if (pose === "idle") {
    const a = t * 0.04;
    [[Math.cos(a)*8, Math.sin(a)*4],[Math.cos(a+1.57)*8, Math.sin(a+1.57)*4],
     [Math.cos(a+3.14)*8, Math.sin(a+3.14)*4],[Math.cos(a+4.71)*8, Math.sin(a+4.71)*4]
    ].forEach(([dx,dy],i) => {
      const col=[PA.cx0,PA.cx1,PA.cxB,PA.cxC][i];
      crystal(Math.round(20+dx), Math.round(22+dy+bob), col, PA.cx2);
    });
  } else if (pose === "working") {
    const a = t * 0.12;
    for (let i=0;i<5;i++) {
    ...
    [[-7,1],[7,-2],[-4,6],[9,3]].forEach(([dx,dy],i) => {
      crystal(20+dx, 22+dy, [PA.cx0,PA.cx1,PA.cxB,PA.cxC][i], PA.cx2);
    });
  }
  ...
  if (pose === "idle") {
    ...
    rect(26, bY+2, 5, 8, PA.r1); rect(26, bY+9, 5, 5, PA.sk); // hand adj glasses
    // Crystals clustering
    [[-5,-5],[-3,-7],[1,-6],[3,-4],[-2,-3],[4,-7]].forEach(([dx,dy],i) => {
      rect(19+dx, bY-2+dy, 2, 2, [PA.cx0,PA.cx1,PA.cxB,PA.cx2,PA.cxC,PA.cx0][i]);
    });
  } else if (pose === "working") {
    ...
    // Text crystals trailing
    const tr = Math.sin(t*0.08)*1.5;
    [[-6,-1],[-9,1],[-12,-2],[-15,0],[-18,-3]].forEach(([dx,dy],i) => {
    ...
    rect(28, bY+4, 4, 3, PA.sk); px(32, bY+5, PA.sk); // pointing
    // Text fragments
    [[2,-2],[4,2],[-2,4]].forEach(([dx,dy]) => { rect(20+dx, bY+dy, 3, 2, PA.tc); px(20+dx, bY+dy, PA.amLt); });
  ...
  if (sl) {
    ...
    if (pose==="thinking") { rect(16, hY+4, 2, 2, c(PA.e)); rect(22, hY+4, 2, 2, c(PA.e)); } // look up
    glow(15, hY+4, 4, 2, c(glassGlow)+"66"); glow(21, hY+4, 4, 2, c(glassGlow)+"66");
  }
  px(18, hY+8, c(PA.skD)); px(19, hY+8, c(PA.skD)); // nose
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// CHARACTER REGISTRY + APP
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const CHARS = [
  { id:"monkey",    emoji:"🐒", label:"Space Monkey",         sub:"Mission Director",          accent:"#9B6FE8", fn:drawMonkey    },
  { id:"ninja",     emoji:"🥷", label:"Life Support Officer", sub:"Infrastructure Controller", accent:"#FF40B0", fn:drawNinja     },
  { id:"engineer",  emoji:"🔧", label:"Systems Engineer",     sub:"Coder / Builder",           accent:"#20D0E8", fn:drawEngineer  },
  { id:"archivist", emoji:"📚", label:"Station Archivist",    sub:"Memory & Knowledge",        accent:"#E0A030", fn:drawArchivist },
];
const POSES = ["IDLE","THINK","WORK","SLEEP","WALK","COLLAB"];
const POSE_KEYS = ["idle","thinking","working","sleep","walk","collab"];

export default function App() {
  const [tick, setTick] = useState(0);
  const [activeChar, setActiveChar] = useState(0);
  const [hoveredPose, setHoveredPose] = useState(0);
  const [exporting, setExporting] = useState(null);
  const tRef = useRef(0);

  const sheetCanvases = CHARS.map(() => useRef(null));
  const displayCanvases = CHARS.map(() => useRef(null));
  ...
  useEffect(() => {
    CHARS.forEach((ch, ci) => {
      const cvs = sheetCanvases[ci].current;
      if (!cvs) return;
      const ctx = cvs.getContext("2d");
      ctx.clearRect(0, 0, 1920, 240);
      POSE_KEYS.forEach((pose, pi) => ch.fn(ctx, pose, pi * FRAME_W, 0, tRef.current));
      // Scale to display canvas
      const disp = displayCanvases[ci].current;
      if (disp) {
        const dctx = disp.getContext("2d");
        dctx.clearRect(0, 0, disp.width, disp.height);
        dctx.imageSmoothingEnabled = false;
        dctx.drawImage(cvs, 0, 0, disp.width, disp.height);
      }
    });
  }, [tick]);
  ...
  useEffect(() => {
    ...
    CHARS[activeChar].fn(ctx, POSE_KEYS[hoveredPose], 0, 0, tRef.current);
  }, [tick, activeChar, hoveredPose]);

  function exportPNG(ci) {
    setExporting(ci);
    setTimeout(() => {
      const cvs = sheetCanvases[ci].current;
      if (!cvs) { setExporting(null); return; }
      const a = document.createElement("a");
      a.download = `${CHARS[ci].id}-spritesheet-1920x240.png`;
  ...
  const acc = CHARS[activeChar].accent;

  return (
    <div style={{ minHeight:"100vh", background:"#12182A", color:"#D0C8F0", fontFamily:"'Courier New', monospace", padding:"18px 14px 40px", boxSizing:"border-box" }}>
      {CHARS.map((_,ci) => <canvas key={ci} ref={sheetCanvases[ci]} width={1920} height={240} style={{display:"none"}} />)}

      {/* Header */}
      <div style={{textAlign:"center",marginBottom:22}}>
        <div style={{fontSize:9,letterSpacing:6,color:"#3A3468",textTransform:"uppercase",marginBottom:3}}>Deep Space Station · Crew Registry</div>
        <h1 style={{margin:0,fontSize:18,fontWeight:900,letterSpacing:3,color:"#F0ECF8"}}>SPRITE SHEET GENERATOR</h1>
        <div style={{fontSize:9,color:"#2C2848",marginTop:3,letterSpacing:4}}>16-BIT PIXEL ART · 6 POSES · TRUE TRANSPARENCY · 1920×240</div>
      </div>

      {/* Character tabs */}
      <div style={{display:"flex",gap:5,justifyContent:"center",marginBottom:18,flexWrap:"wrap"}}>
        {CHARS.map((ch,i) => (
          <button key={ch.id} onClick={() => setActiveChar(i)} style={{
            background: activeChar===i ? ch.accent+"18" : "transparent",
            border:`1.5px solid ${activeChar===i ? ch.accent : "#1A183A"}`,
            color: activeChar===i ? ch.accent : "#504888",
            padding:"6px 13px", borderRadius:4, cursor:"pointer",
            fontFamily:"inherit", fontSize:11, letterSpacing:1.5, transition:"all 0.15s",
          }}>{ch.emoji} {ch.id.toUpperCase()}</button>
        ))}
      </div>

      {/* Preview + pose picker */}
      <div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:22,flexWrap:"wrap",alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:8,color:"#2C2848",letterSpacing:4,marginBottom:5,textAlign:"center"}}>LIVE PREVIEW · {POSES[hoveredPose]}</div>
          <div style={{border:`1px solid ${acc}2A`,borderRadius:7,overflow:"hidden",background:"#080C18",lineHeight:0}}>
            <canvas ref={previewRef} width={320} height={240} style={{display:"block",imageRendering:"pixelated"}} />
          </div>
          <div style={{textAlign:"center",marginTop:6,fontSize:10,color:acc,letterSpacing:1}}>{CHARS[activeChar].emoji} {CHARS[activeChar].label}</div>
          <div style={{textAlign:"center",fontSize:8,color:"#383460",letterSpacing:1}}>{CHARS[activeChar].sub}</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:7}}>
          <div style={{fontSize:8,color:"#2C2848",letterSpacing:4,marginBottom:1}}>SELECT POSE</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
            {POSES.map((p,pi) => (
              <button key={p} onMouseEnter={() => setHoveredPose(pi)} onClick={() => setHoveredPose(pi)} style={{
                padding:"7px 11px", background: hoveredPose===pi ? acc+"12" : "transparent",
                border:`1px solid ${hoveredPose===pi ? acc : "#16143A"}`,
                color: hoveredPose===pi ? acc : "#484870",
                borderRadius:3, cursor:"pointer", fontFamily:"inherit", fontSize:10, letterSpacing:1.5, transition:"all 0.1s", textAlign:"left",
              }}><span style={{opacity:0.4,fontSize:8}}>0{pi+1} </span>{p}</button>
            ))}
          </div>
          <button onClick={() => exportPNG(activeChar)} disabled={exporting!==null} style={{
            marginTop:2,padding:"9px 14px",background:acc+"14",
            border:`1.5px solid ${acc}`,color:acc,borderRadius:4,
            cursor:"pointer",fontFamily:"inherit",fontSize:10,letterSpacing:2,transition:"all 0.14s",
          }}>{exporting===activeChar ? "EXPORTING…" : "↓ EXPORT 1920×240 PNG"}</button>
          <div style={{fontSize:8,color:"#1E1C38",letterSpacing:1,textAlign:"center"}}>transparent · anti-aliased for #12182A</div>
        </div>
      </div>

      {/* All 4 strips */}
      <div style={{fontSize:8,color:"#1E1C38",letterSpacing:4,marginBottom:12,textAlign:"center"}}>ALL SPRITE SHEETS — EXPORT FOR FULL 1920px</div>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        {CHARS.map((ch,ci) => (
          <div key={ch.id}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
              <span style={{fontSize:10,color:ch.accent,letterSpacing:1.5}}>{ch.emoji} {ch.label.toUpperCase()}</span>
              <span style={{fontSize:8,color:"#2C2848"}}>— {ch.sub}</span>
              <button onClick={() => exportPNG(ci)} disabled={exporting!==null} style={{
                marginLeft:"auto",padding:"2px 9px",background:"transparent",
                border:`1px solid ${ch.accent}44`,color:ch.accent,borderRadius:3,
                cursor:"pointer",fontFamily:"inherit",fontSize:9,letterSpacing:1,
              }}>↓ PNG</button>
            </div>
            <div style={{display:"flex",marginBottom:2}}>
              {POSES.map(p => <div key={p} style={{flex:"0 0 96px",minWidth:96,fontSize:7,color:"#1E1C38",letterSpacing:2,textAlign:"center"}}>{p}</div>)}
            </div>
            <div style={{overflowX:"auto",borderRadius:3,border:`1px solid ${ch.accent}1A`}}>
              <div style={{width:576,background:"#080C18",lineHeight:0}}>
                <canvas ref={displayCanvases[ci]} width={576} height={72} style={{display:"block",imageRendering:"pixelated",width:"100%"}} />
              </div>
            </div>
          </div>
        ))}
      </div>
      <div style={{textAlign:"center",marginTop:24,fontSize:8,color:"#141230",letterSpacing:2}}>CLICK ↓ PNG TO DOWNLOAD FULL 1920×240 SPRITE SHEET WITH TRUE ALPHA TRANSPARENCY</div>
    </div>
  );
}
